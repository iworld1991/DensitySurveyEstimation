...
sim_probs=np.array([0,0.2,0.3,0.1,0.4])
...
sim_n=1000
true_alpha,true_beta,true_loc,true_scale=0.4,0.8,0,1
...
for i in range(nobs):
    ...
    if not np.isnan(PRCCPI_y0).any():
        ...
        if len(stats_est)>0:
            IndSPF_moment_est['Mean'][i]=stats_est['mean']
            print(stats_est['mean'])
            IndSPF_moment_est['Var'][i]=stats_est['variance']
            print(stats_est['variance'])
            IndSPF_moment_est['Std'][i]=stats_est['std']
            print(stats_est['std'])
            IndSPF_moment_est['Iqr1090'][i]=stats_est['iqr1090']
...
IndSPF_pk['Mean']=pd.to_numeric(IndSPF_pk['Mean'],errors='coerce')   # CPI from y-1 to y 
IndSPF_pk['Var']=pd.to_numeric(IndSPF_pk['Var'],errors='coerce')  
IndSPF_pk['Std']=pd.to_numeric(IndSPF_pk['Std'],errors='coerce') 
IndSPF_pk['Iqr1090']=pd.to_numeric(IndSPF_pk['Iqr1090'],errors='coerce') 


IndSPF_pk.tail()

IndSPF_pk.to_excel('../data/Dstsample_data.xlsx')

# + code_folding=[]
### Robustness checks: focus on big negative mean estimates 
sim_bins_data = SPF_bins
print(str(sum(IndSPF_pk['Mean']<=0))+' abnormals')
ct=0
figure=plt.plot()
for id in IndSPF_pk.index[IndSPF_pk['Mean']<-2]:
    print(id)
    print(IndSPF_pk['Mean'][id])
    sim_probs_data= np.flip(np.array([IndSPF['PRCCPI'+str(n)][id]/100 for n in range(1,11)]))
    plt.bar(sim_bins_data[1:],sim_probs_data)
    print(sim_probs_data)
    stats_est=SynDensityStat(SPF_bins,
                             sim_probs_data)
    print(stats_est['mean'])
# -
